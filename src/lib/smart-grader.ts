...
function buildAiPrompt(needAI: WritingAnswer[]): string {
  ...
  lines.push('- Different notation: a^7 = aaaaaaa (a multiplied 7 times), 2^10 = 1024, 1/2 = 0.5 = ½, x^(1/2) = √x, √50 = 5√2')
  ...
  lines.push('- Extract the FINAL value from both sides (usually after the last =) and compare ONLY the final values.')
  lines.push('')
  lines.push('Scoring rules:')
  lines.push('- Final value mathematically equal → full points, isCorrect: true')
